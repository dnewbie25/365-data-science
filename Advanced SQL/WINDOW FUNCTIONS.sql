...
select emp_no, salary, rank() over w as rank_num from salaries where emp_no = 11839
window w as (partition by emp_no order by salary desc);

select emp_no, salary, rank() over (partition by emp_no order by salary desc) from salaries;